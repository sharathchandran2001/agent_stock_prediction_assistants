...
agent_configs = {
    ...
    "stock_agent_4": {
        "label": "Global Market Trends",
        "filename": "stock_agent4.txt",
        "instructions": "Analyze global market trends and provide insights."
    },
    "stock_agent_5": {
        "label": "Economic Indicators Analysis",
        "filename": "stock_agent5.txt",
        "instructions": "Analyze economic indicators and their impact on the stock market."
    },
    "stock_agent_6": {
        "label": "Investor Sentiment Analysis",
        "filename": "stock_agent6.txt",
        "instructions": "Analyze investor sentiment and its influence on stock performance."
    },
    "stock_agent_7": {
        "label": "Technical Analysis Insights",
        "filename": "stock_agent7.txt",
        "instructions": "Provide technical analysis insights for stock predictions."
    },
    "stock_agent_8": {
        "label": "Industry-Specific Trends",
        "filename": "stock_agent8.txt",
        "instructions": "Analyze industry-specific trends and their impact on stock performance."
    },
    "stock_agent_context": {
...
for key, cfg in agent_configs.items():
    ...
    default_text = read_text(input_path)
...
if st.button("Generate Stock Prediction"):
    ...
    for key, cfg in agent_configs.items():
        if key == "final_stock_agent":
            continue
        input_text = input_data.get(key, "").strip()
        if input_text:  # Process only if input is non-empty
    ...
    combined_prompt = "\n\n".join(
        f"[{cfg['label']}]\n{section_outputs[key]}" for key, cfg in agent_configs.items()
        if key != "final_stock_agent" and key in section_outputs
    )
